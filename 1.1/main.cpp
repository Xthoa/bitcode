...
#include<conio.h>
#define elif else if
vector<string> pkg={"prime.pkg","io.pkg"};
class err{
	public:
	string s;
	err(string r){
...
int import(string str){
	...
	int ret;
	cout<<"Package Local Identifier : "<<(ret=packs.size())<<endl;
	int pos=*(short*)(p.buf+1);
	int globs=*(short*)(p.buf+pos);
	cout<<"Package Global Symbol Count : "<<globs<<endl;
	for(short* tp=(short*)(p.buf+pos+2);globs;globs--,tp++)p.globs.push_back((int)*tp);
	packs.push_back(p);
	return ret;
}
...
bool runcode(char c){
	...
	elif(code==0x0a or code==0x90/*nasm align*/)asm("nop");
	...
	elif(code==0x16)cout<<data4(read1());
	elif(code==0x17)cin>>data2(read1());
	elif(code==0x18)cout<<data2(read1());
