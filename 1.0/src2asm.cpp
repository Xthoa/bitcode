...
int main(int argc,char** argv){
	...
	auto calcs=[bits](int c){
		int n=0x57;
		if(bits==32)n+=0x0c;
		int src;fin>>src;
		string dst=getword();
		string b=(bits==32?"dd ":"dw ");
		if(dst[0]=='*'){
			n-=0x30;
			dst=dst.substr(1);
			b="db ";
	...
	while(1){
		...
		if(cmd=="exit")cout<<"db 0x09"<<endl,adr++;
		...
		elif(cmd=="puts")cout<<"db 0x88,"<<getword()<<endl;
		elif(cmd=="putsl"){
			string cont=getword();
			cout<<"db 0x89,"<<cont.length()<<"\n\tdb \""<<cont<<"\""<<endl;
		}
		elif(cmd=="call")cout<<"db "<<(bits==32?"0x95":"0x94")<<"\n\t"<<(bits==32?"dd ":"dw ")<<getword()<<endl;
		elif(cmd=="ret" or cmd=="return")cout<<"db 0x96"<<endl;
		elif(cmd=="push")cout<<"db "<<(bits==32?"0x8c":"0x8b")<<"\n\t"<<(bits==32?"dd ":"dw ")<<getword()<<endl;
		elif(cmd=="pop")cout<<"db "<<(bits==32?"0x8f,":"0x8e,")<<getword()<<endl;
		elif(cmd=="pushm")cout<<"db "<<(bits==32?"0x98,":"0x97,")<<getword()<<endl;
		elif(cmd=="putsp"){
			s=getword();
			cout<<"db "<<(bits==32?"0x91,":"0x90,")<<s<<"\n\t"<<(bits==32?"dd ":"dw ")<<getword()<<endl;
		}
		elif(cmd=="putspm"){
			s=getword();
			...
			s=getword();cout<<"db 0x9c,"<<s<<","<<getword()<<endl;
		}
		elif(cmd=="je"){
			...
			s=getword();cout<<"db 0x9d,"<<s<<","<<getword(),jump();
		}
		elif(cmd=="jlm"){
			s=getword();cout<<"db 0x9e,"<<s<<","<<getword(),jump();
		}
		elif(cmd=="jgm"){
			s=getword();cout<<"db 0x9f,"<<s<<","<<getword(),jump();
		}
		elif(cmd=="jnem"){
			s=getword();cout<<"db 0xa0,"<<s<<","<<getword(),jump();
		}
		elif(cmd=="jngm" or cmd=="jlem"){
			s=getword();cout<<"db 0xa1,"<<s<<","<<getword(),jump();
		}
		elif(cmd=="jnlm" or cmd=="jgem"){
			s=getword();cout<<"db 0xa2,"<<s<<","<<getword(),jump();
		}elif(cmd=="import"){
			s=getword();
			...
			s=getword();
			cout<<"db 0xab,"<<s<<"\n\tdd "<<getword()<<endl;
		}elif(cmd=="package")cout<<"db 0x08"<<endl;
